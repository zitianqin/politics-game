...
export default function RevealPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  const topRow = selectedVoters.slice(0, 2);
  const bottomRow = selectedVoters.slice(2, 5);

  const VoterCard = ({ voter }: { voter: typeof selectedVoters[0] }) => (
    <div
  ...
  return (
    <div className="min-h-screen bg-[#6149D2] bg-[radial-gradient(#ffffff33_1px,transparent_1px)] [background-size:20px_20px] flex flex-col items-center p-8 overflow-y-auto font-sans">
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-6 px-16">
          {topRow.map((voter) => <VoterCard key={voter.id} voter={voter} />)}
        </div>

        {/* Bottom row — 3 cards */}
        <div className="grid grid-cols-3 gap-6">
          {bottomRow.map((voter) => <VoterCard key={voter.id} voter={voter} />)}
        </div>
      </motion.div>
