...
- During game start, randomly draw 21 distinct voter profiles (or seed it per game code if decided).
...
**Assignee:** Frontend
**Details:**
- Implement `/api/game/:code/start` (Host triggers game start, initiates generation of 2 candidates, 21 voters, 2 topics). Push `game:start` socket event `{ candidates, voters, topics }`.
- Route: `/reveal/[code]`
- Build `CandidateCard` component to show the player's own and the opponent's full candidate bio.
- Build `VoterGrid` component to display the 21 randomly selected active voters out of 67. The remaining 46 are strictly excluded.
...
- Payload inclusions: Total accumulated transcript (all rounds so far), all 21 active voters, both candidates, topics discussed.
- JSON enforcement string. Only parse the JSON map of 21 voter key-values returning: `{ vote, reason }`.
...
- Calculate tallies. Check if all 21 voters returned a value. Reconstruct final breakdown.
